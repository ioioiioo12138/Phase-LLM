...
def launch():
    # 当作为脚本被 torchrun 直接调用时，__package__ 为空，需要手动补 pythonpath
    if __package__ is None:  # pragma: no cover
        sys.path.insert(0, str(Path(__file__).resolve().parents[1]))  # .../LLaMA-Factory/src
    ...
    # torchrun 会在 argv[1] 注入 --local_rank/--node_rank 等参数，先剔除它们再取子命令
    while len(sys.argv) > 1 and sys.argv[1].startswith("--local_rank"):
        sys.argv.pop(1)

    command = sys.argv.pop(1) if len(sys.argv) > 1 else "help"
    already_in_torchrun = os.getenv("LOCAL_RANK") is not None or os.getenv("RANK") is not None
    if is_env_enabled("USE_MCA"):  # force use torchrun
        os.environ["FORCE_TORCHRUN"] = "1"

    if command == "train" and not already_in_torchrun and (
        ...
        # torchrun 重新进入本文件时，sys.argv 不再包含最初的 "train"；
        # 需要手动补回，否则子进程会把配置文件名当成 command，直接退出。
        forward_args = ["train"] + sys.argv[1:]
        ...
        # 尽量稳健地解析 YAML，即使 argv 被 torchrun/launcher 重排也能找到配置
        print(f"[launcher] argv={sys.argv}", flush=True)
        config_path = None
        for a in sys.argv[1:]:
            if a.endswith((".yaml", ".yml")):
                config_path = Path(a).absolute()
                break

        if config_path is not None and config_path.exists():
            print(f"[launcher] using config={config_path}", flush=True)
            override_cli = [x for x in sys.argv[1:] if x != str(config_path)]
            merged = OmegaConf.merge(OmegaConf.load(config_path), OmegaConf.from_cli(override_cli))

            # HfArgumentParser 不接受裸 key（如文件名）; 移除无关键
            merged.pop(config_path.stem, None)

            run_exp(args=OmegaConf.to_container(merged, resolve=True))
        else:
            if config_path:
                print(f"[launcher] config not found: {config_path}", flush=True)
            run_exp()
...
if __name__ == "__main__":
    # When invoked directly (e.g., via torchrun), reuse the launcher logic above
    launch()
