...
def material_accuracy_reward(prompts, completions, answer, **kwargs):
    """
    针对高熵合金相位预测任务的准确性奖励函数。
    它会检查模型输出的最后部分是否包含数据集中指定的三个标准短语之一。
    """
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    ...
    for content, ground_truth in zip(contents, answer):
        # 1. 提取 <think> 之后的内容（或者直接看最后几行）
        # 我们假设答案出现在思考过程之后
        clean_content = content.split("</think>")[-1].strip()
        
        # 2. 检查 ground_truth (即 jsonl 中的 response 字段提取出的结论)
        # 注意：你需要确保传入的 answer 是预处理过的标准结论短语
        
        # 匹配逻辑：如果 clean_content 中包含了正确的短语，则给 1.0 分
        if ground_truth.strip() in clean_content:
            rewards.append(1.0)
        else:
            rewards.append(0.0)
...
def material_format_reward(prompts, completions, **kwargs):
    """
    检查模型是否遵循了特定的输出格式：<think>...</think> + 结论
    """
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    
    for content in contents:
        # 检查是否包含 think 标签且标签后有内容
        pattern = r"<think>.*?</think>\s*\n\n.+"
        if re.search(pattern, content, re.DOTALL):
            rewards.append(0.5) # 格式正确给 0.5 分
        else:
            rewards.append(0.0)
